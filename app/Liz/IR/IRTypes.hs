...
instance PP.Pretty IROp where
  ...
  pretty (IRFunc i args body retTy) =
    (PP.pretty @T.Text i) <> (PP.encloseSep PP.lparen PP.rparen (PP.comma <> PP.space) $ map prettifyArg args) 
      PP.<+> (PP.pretty @T.Text "->") PP.<+> (PP.viaShow retTy) 
        <> (PP.pretty @T.Text ":") <> PP.line 
          <> (PP.indent 2 . PP.vcat $ map PP.pretty body)
  ...
  pretty (IRLabel (Label (name, exprs))) =
    (PP.pretty @T.Text $ name <> ":") 
      <> PP.line <> (PP.indent 2 . PP.vcat $ map PP.pretty exprs) 
        <> PP.line
  pretty (IRGoto name) = (PP.pretty @T.Text "goto") PP.<+> (PP.pretty name)
  pretty (IRFuncCall i exprs) = 
    (PP.pretty @T.Text i) 
      <> (PP.parens . PP.hsep . PP.punctuate PP.comma $ map PP.pretty exprs)
...
formatVar dec ident v@(IRVar i _) = 
  (PP.pretty v) <> PP.line 
    <> (PP.pretty dec) PP.<+> (PP.pretty ident) 
      PP.<+> (PP.pretty @T.Text "=") PP.<+> (PP.pretty i)
formatVar dec ident v@(IRConst i _) = 
  (PP.pretty v) <> PP.line 
    <> (PP.pretty dec) PP.<+> (PP.pretty ident) 
      PP.<+> (PP.pretty @T.Text "=") PP.<+> (PP.pretty i)
formatVar dec ident v = 
  (PP.pretty @T.Text dec) PP.<+> (PP.pretty ident) 
    PP.<+> (PP.pretty @T.Text "=") PP.<+> (PP.pretty v)

formatBinary :: L.BinaryOp -> IROp -> IROp -> PP.Doc ann
formatBinary op l@(IRVar li _) r@(IRVar ri _) = 
  (PP.pretty l) <> PP.line <> (PP.pretty r) 
    <> PP.line <> (PP.pretty li) 
      PP.<+> (PP.pretty $ binaryToText op) 
        PP.<+> (PP.pretty ri)
...
formatBinary op l@(IRVar li _) r = 
  (PP.pretty l) <> PP.line <> (PP.pretty li) 
    PP.<+> (PP.pretty $ binaryToText op) 
      PP.<+> (PP.pretty r)
...
formatBinary op l r@(IRVar ri _) = 
  (PP.pretty r) <> PP.line <> (PP.pretty l) 
    PP.<+> (PP.pretty $ binaryToText op) 
      PP.<+> (PP.pretty ri)
...
formatIfStmt cond@(IRVar ident _) gotomain (gototrue, lbl@(Label _)) Nothing =
  (PP.pretty cond) <> PP.line 
    <> (PP.pretty @T.Text "if") 
      PP.<+> (PP.pretty ident) PP.<+> (PP.pretty @T.Text "then goto") 
        PP.<+> (PP.pretty gototrue) PP.<+> (PP.pretty @T.Text "else goto") 
          PP.<+> (PP.pretty gotomain) <> PP.line 
            <> (PP.pretty $ IRLabel lbl)
formatIfStmt cond@(IRConst ident _) gotomain (gototrue, lbl@(Label _)) Nothing =
  (PP.pretty cond) <> PP.line 
    <> (PP.pretty @T.Text "if") PP.<+> (PP.pretty ident) 
      PP.<+> (PP.pretty @T.Text "then goto") PP.<+> (PP.pretty gototrue) 
        PP.<+> (PP.pretty @T.Text "else goto") PP.<+> (PP.pretty gotomain) 
          <> PP.line <> (PP.pretty $ IRLabel lbl)
formatIfStmt cond gotomain (gototrue, lbl@(Label _)) Nothing =
  (PP.pretty @T.Text "if") PP.<+> (PP.pretty cond) 
    PP.<+> (PP.pretty @T.Text "then goto") PP.<+> (PP.pretty gototrue) 
      PP.<+> (PP.pretty @T.Text "else goto") PP.<+> (PP.pretty gotomain) 
        <> PP.line <> (PP.pretty $ IRLabel lbl)
formatIfStmt cond@(IRVar i _) _ (gototrue, tlbl@(Label _)) (Just (gotofalse, flbl@(Label _))) =
  (PP.pretty cond) <> PP.line 
    <> (PP.pretty @T.Text "if") PP.<+> (PP.pretty i) 
      PP.<+> (PP.pretty @T.Text "then goto") PP.<+> (PP.pretty gototrue) 
        PP.<+> (PP.pretty @T.Text "else goto") PP.<+> (PP.pretty gotofalse) 
          <> PP.line <> (PP.pretty $ IRLabel tlbl) 
            PP.<+> (PP.pretty $ IRLabel flbl)
